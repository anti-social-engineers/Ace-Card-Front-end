...
const headRows = [
  { id: 'transactie', numeric: false, disablePadding: true, label: 'Transactie ID' },
  { id: 'bedrag', numeric: true, disablePadding: false, label: 'Bedrag' },
  { id: 'club', numeric: false, disablePadding: false, label: 'Club' },
...
export class Transactions extends Component {
  ...
  componentDidMount = () => {
    axios.get(config.API_URL + 'api/account/payments/desc', { headers: { Authorization: this.header } })
      .then(res => {
        this.setState({ rows: res.data.payments, next: res.data.next_cursor }, () => console.log(this.state.rows));
      }).catch(err => {
        console.log(err)
      })
  }
  ...
  loadMore = (next) => {
    axios.get(config.API_URL + 'api/account/payments/desc/' + next, { headers: { Authorization: this.header } })
      .then(res => {
        var current_rows = this.state.rows;
        current_rows.push(...res.data.payments);
